...
class splash:
	def __init__(self):
		gladeUI = resin_config.gladeUI #get glade interface
		gladeUI.signal_autoconnect(self)
		logo = gtk.gdk.pixbuf_new_from_file(resin_config.images['splash'])
		image = gtk.Image()
...
class error_report:
	...
	def generate_model(self,array):
		...
		for error in array:
			...
			if error[0] == 2:
				icon= gtk.gdk.pixbuf_new_from_file(resin_config.images['infoIcon'])
				type = "INFORMATION"
				report = "<span foreground='blue'><b>%s: %s</b>\n %s</span>"%(type,script.title,error[2])
			if error[0] == 1:
				icon= gtk.gdk.pixbuf_new_from_file(resin_config.images['warningIcon'])
				type = "FATAL ERROR"
				report = "<span foreground='red'><b>%s: %s</b>\n %s</span>"%(type,script.title,error[2])
			if error[0] == 0:
				icon= gtk.gdk.pixbuf_new_from_file(resin_config.images['fatalIcon'])
				type = "WARNING"
